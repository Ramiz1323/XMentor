...
const CommunityList = () => {
  ...
  const handleJoin = async (id) => {
    ...
    try {
      ...
      alert(err.message || 'Mission join failed');
    } finally {
  ...
  const handleLeave = async (id) => {
    ...
    try {
      ...
      alert(err.message || 'Mission exit failed');
    } finally {
      setActionLoading(prev => ({ ...prev, [id]: false }));
    }
  };
    ...
    <div className="community-page">
      ...
      {loading ? (
        ...
        <div className="community-grid">
          {filtered.length === 0 ? (
            <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '4rem', opacity: 0.5 }}>No missions found in this sector.</div>
          ) : (
